	...
	while{vehicleSelection<5} do
	{
		...
		while{weatherLoop < 1} do
		{
			...
			while{loop < 2} do
			{
				...
				//CAMERA POS 1
				// dx = [50,1000] call BIS_fnc_randomInt;
				// dy = [50,1000] call BIS_fnc_randomInt;

				_pos = [_vehicle, 50, 500, 0, 0, 40, 0] call BIS_fnc_findSafePos;
				//_cameraPosition = (position _vehicle) vectorAdd [dx, dy, 4]; // Set the camera 10 meters to the side and 5 meters higher than the vehicle
				...
				// Check if the line between the vehicle and the observer intersects any surface
