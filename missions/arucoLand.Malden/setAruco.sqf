0 = [] spawn
{
    ...
    // Create Tank
    tank_l = createSimpleObject["Leopard2A6HEL_2", (ATLToASL [6900,3800,3])];
	sleep 1;
	tank_l setVectorDirAndUp [[1,0,0], [0,0,1]];
    ...
    while{ring<4} do
    {
        ...
        while{num<12} do
        {
			...
			if(ring==0) then
			{
				// Create small targets on surface
				pos = [(x_arr_close select num), (y_arr_close select num), (z_arr select ring)] vectorAdd [(position tank_l) select 0,(position tank_l) select 1,0];
				block_l = createVehicle ["UserTexture1m_F",pos,[],0,"FLY"]; //"Land_VR_Block_04_F"
			}
			else
			{
				// Create large targets on ASL coordinates
				pos = [(x_arr_far select num), (y_arr_far select num), (z_arr select ring)] vectorAdd position tank_l;
				block_l = createVehicle ["UserTexture10m_F",pos,[],0,"FLY"]; //"Land_VR_Block_04_F"
				asl_pos = getPosASL tank_l;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Create Tank
    ship_l = createSimpleObject["HAFM_MEKO_HN", ( [9100,5000,12])];
	sleep 1;
