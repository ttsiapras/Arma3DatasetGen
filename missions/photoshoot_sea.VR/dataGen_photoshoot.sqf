0 = [] spawn
{
	...
	imageLoop   = 10; //Number of images per vehicle per position
	...
	while{(vehicleSelection<nof_vehicles)} do
	{
		...
		{
			//Create the vehicle
			_v_spawn_pos  = [[100,7800] call BIS_fnc_randomInt,[100,7800] call BIS_fnc_randomInt];
			_vehicle = createVehicle [_vehicleName, _v_spawn_pos, [], 0, "NONE"]; // Create a vehicle (MRAP for example)

			// Set camera position
			_pos = [_vehicle, 30, offset_max, 0, 0, 40, 0] call BIS_fnc_findSafePos;
