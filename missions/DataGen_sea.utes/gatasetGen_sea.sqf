0 = [] spawn
{
	...
	imageLoop   = 4; //Number of images per vehicle per position
	...
	while{(vehicleSelection<nof_vehicles)} do
	{
		...
		while{pos_count<nof_positions} do
		{
			...
			while{loop < imageLoop  && failed<1} do
			{
				...
				private _pos =_v_spawn_pos + [0];
				_vehicle = createVehicle [_vehicleName, [0,0,0], [], 0, "CAN_COLLIDE"]; // Create a vehicle (MRAP for example)
				_vehicle setPosASL _pos;
				_vehicle setVectorDirAndUp [[1,0,0], [0,0,1]];
				_vehicle enableSimulationGlobal false;  // freezes it completely
				_vehicle allowDamage false;             // optional: make it indestructible
				// Set camera position
				_pos = [_vehicle, 400, 500, 0, 2, 40, 0] call BIS_fnc_findSafePos;
				elevation = [5,20] call BIS_fnc_randomInt;
